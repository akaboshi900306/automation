...
from (
  select DIVISION
       ...
       , SAFE_DIVIDE(sum(JOB_COUNT_SCHEDULED) - sum(JOB_COUNT_INSTALLED), sum(JOB_COUNT_SCHEDULED)) as FALL_OFF_PCT_OF_JOB_COUNT
  from `analytics-ahs-owned-thd.BSC.HDE_FALLOFF_REGION`
    group by 1,2,3
  ...
  select 'Grand Total' as DIVISION
     , 'Grand Total' as LOCATION_LEVEL
     , 'Grand Total' as LOCATION
     , sum(HDIS_SCORECARD_IR) as HDIS_SCORECARD_IR
     , sum(HDIS_SCORECARD_JOB_COUNT) as HDIS_SCORECARD_JOB_COUNT
     , sum(JOB_AMT_SCHEDULED) as BOW_SCHEDULED
     , sum(JOB_COUNT_SCHEDULED) as BOW_SCHEDULED_JOBS
